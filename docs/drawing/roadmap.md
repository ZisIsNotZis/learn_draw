...
| --- | --- | --- | --- |
| **G8** | **the rubric (PRIMARY)** | `draw check` bundle (1:1 draft FIRST, then the side-by-side, then the region crop) → two independent, sight-proven reviewers score the **eleven checkable items across six axes** in `docs/drawing/rubric.md` v2 | the milestone's **named axes** reach their targets on both reviewers, with every axis spread ≤ 1 (axes differing by ≥ 2 are *contested* and cannot pass a milestone until settled) |
| **G8b** | **contextual similarity alarm** | `.venv/bin/python scripts/similarity.py <render>` | the CLIP cosine similarity to the reference has not fallen by more than 0.02 since the last rubric review. **A secondary alarm, never a gate** — see rubric Part III for why a contextual scalar must not be optimised |
